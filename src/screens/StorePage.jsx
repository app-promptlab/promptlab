...
import { ShoppingBag, Images } from 'lucide-react'; // <--- CORREÇÃO: Importei Images

export default function StorePage({ packs, onPurchase }) {
    const [localPacks, setLocalPacks] = React.useState(packs || []);
    
    // Fetch interno caso venha vazio do pai
    React.useEffect(() => {
        if(!packs || packs.length === 0) {
             // Import dinâmico para evitar ciclo, ou usar prop passada. 
             // Para simplificar, assumimos que o componente pai (App) ou o próprio StorePage faz o fetch.
             // Vamos deixar visual por enquanto.
        } else {
            setLocalPacks(packs);
        }
    }, [packs]);
