  ...
  <div className="relative w-full h-full overflow-hidden rounded-xl bg-black/50 group">
    ...
    <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-24 h-32">
            <svg width="0" height="0"><defs><clipPath id="lockClip" clipPathUnits="objectBoundingBox"><path d="M0.5,0 C0.35,0,0.22,0.12,0.22,0.28 V0.4 H0.15 C0.07,0.4,0,0.47,0,0.56 V0.9 C0,0.98,0.07,1,0.15,1 H0.85 C0.93,1,1,0.98,1,0.9 V0.56 C1,0.47,0.93,0.4,0.85,0.4 H0.78 V0.28 C0.78,0.12,0.65,0,0.5,0 M0.5,0.78 C0.45,0.78,0.4,0.73,0.4,0.68 C0.4,0.63,0.45,0.58,0.5,0.58 C0.55,0.58,0.6,0.63,0.6,0.68 C0.6,0.73,0.55,0.78,0.5,0.78 M0.35,0.4 V0.28 C0.35,0.2,0.42,0.12,0.5,0.12 C0.58,0.12,0.65,0.2,0.65,0.28 V0.4 H0.35" /></clipPath></defs></svg>
...
export default function PromptsGallery({ user, showToast }) {
  ...
  // Estado para armazenar IDs dos favoritos (Set para busca rápida)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // 2. Carregar Favoritos do Usuário
      if (user) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-fadeIn px-4 pb-20 pt-8">
      ...
      {/* PROMPTS */}
      <div>
        <h2 className="text-2xl font-bold text-white mb-6 flex items-center"><Sparkles className="mr-2 text-blue-500" size={20} /> {selectedPack ? selectedPack.title : 'Feed de Prompts'}</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
          {filteredPrompts.map((item, index) => {
             ...
             // Verifica se está favoritado para exibir o coração vermelho fixo
             const isLiked = likedIds.has(item.id);
             ...
             return (
               <div key={item.id} onClick={() => setModalItem(item)} className="aspect-[3/4] rounded-xl overflow-hidden relative group cursor-pointer bg-black">
                 ...
                 {/* Coração FIXO se estiver favoritado, ou no HOVER se não estiver */}
                 <button 
                    onClick={(e) => toggleFavorite(e, item)} 
                    className={`absolute top-2 right-2 p-2 rounded-full transition-all duration-300 z-20 
                        ${isLiked ? 'bg-black/50 opacity-100 text-red-500 scale-110' : 'text-white opacity-0 group-hover:opacity-100 hover:text-red-500 hover:bg-black/50'}
                    `}
                 >
