...
// --- Componente Input de Cor (Estilizado) ---
...
export default function AdminPanel({ showToast }) {
  ...
  // --- ACTIONS: SALVAR IDENTIDADE E HEADER ---
  ...
  // --- [CORRIGIDO] SALVAR BLOCO (BANNER/VIDEO) ---
  const saveBlock = async (e) => {
    e.preventDefault();
    const payload = { ...editingBlock, page_id: selectedPage };
    
    // Se for novo, define ordem
    if (!payload.id && !payload.order_index) payload.order_index = pageContent.length + 1;
    ...
    if (payload.id) {
        ...
        // Garante que a propriedade 'id' não existe para o banco criar
        delete payload.id;
  ...
  // --- [CORRIGIDO] SALVAR ITEM (PACK/PROMPT) ---
