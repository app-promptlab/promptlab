...
export default function AdminPanel({ showToast }) {
  ...
  // --- SALVAR CONFIGS (Identity e Header) ---
  const saveIdentity = async () => { 
      // Garante que só faz update
  ...
  // --- SALVAR BLOCOS E ITENS (CORREÇÃO DE ID) ---
  const saveGeneric = async (e, isBlock = false) => {
    e.preventDefault();
    let payload = isBlock ? { ...editingBlock, page_id: selectedPage } : { ...editingItem };
    let table = isBlock ? 'page_content' : (selectedPack ? 'pack_items' : 'products');

    // Se é NOVO (não tem ID ou ID é 0), remove o campo ID para o banco gerar
    if (!payload.id || payload.id === 0) {
        delete payload.id;
        if (isBlock) payload.order_index = pageContent.length;
    ...
    if (!error) { 
        showToast("Salvo!"); 
        if (isBlock) { setEditingBlock(null); loadPageData(selectedPage); }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 pb-32 animate-fadeIn">
      ...
      {/* === ABA 1: EDITOR VISUAL (LAYOUT E IDENTIDADE) === */}
      {activeTab === 'editor' && (
        <div className="max-w-6xl space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {/* Coluna 2: Conteúdo (Vídeos/Banners) */}
      ...
      {/* === ABA 2: GERENCIAR PACKS (PRODUTOS) === */}
      {activeTab === 'prompts' && (
        <div>
      ...
      {editingItem && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-gray-900 w-full max-w-2xl rounded-2xl border border-gray-700 p-6 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={(e) => saveGeneric(e, false)} className="overflow-y-auto custom-scrollbar space-y-4">
      ...
      {editingBlock && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-gray-900 w-full max-w-lg rounded-xl border border-gray-700 p-6 space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4"><button onClick={() => setEditingBlock(null)} className="px-4 py-2 text-gray-400 text-sm font-bold">Cancelar</button><button onClick={(e) => saveGeneric(e, true)} className="px-6 py-2 bg-blue-600 text-white rounded text-sm font-bold">Salvar Bloco</button></div>
            </div>
        </div>
      )}
    </div>
