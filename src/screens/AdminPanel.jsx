...
export default function AdminPanel({ showToast }) {
  ...
  const TABS = [
    ...
    { id: 'tutorials', label: 'Gerenciar Tutoriais', icon: Play }, // Aulas
  ...
  const [draggedItem, setDraggedItem] = useState(null); // Para Drag & Drop
  ...
  const loadPageData = async (pageId) => {
    ...
    // Carrega blocos (vídeos/banners) ordenados
  ...
  const handleDragStart = (e, index, listType) => { 
    setDraggedItem({ index, type: listType }); 
  ...
  const handleDrop = async (e, dropIndex, listType) => {
    ...
    // Identifica qual lista estamos mexendo
    ...
    if (listType === 'blocks') setPageContent(newList);
    else setItems(newList);

    // Salva no Banco
    const updates = newList.map((item, idx) => ({ id: item.id, order_index: idx }));
    let table = '';
    if (listType === 'blocks') table = 'page_content';
    else if (activeTab === 'prompts') table = 'pack_items';
    else if (activeTab === 'tutorials') table = 'tutorials_videos';
  ...
  const saveIdentity = async () => { 
      // Garante que não está tentando atualizar o ID se ele for novo (embora identity deva ter ID)
      const { id, ...updates } = siteIdentity;
      // Update seguro onde app_name bate ou ID bate
  ...
  const saveBlock = async (e) => {
    ...
    if (!payload.order_index && !payload.id) payload.order_index = pageContent.length + 1; // Novo item vai pro fim
    
    // Remove ID se for novo para o Supabase gerar
  ...
  const handleSaveItem = async (e) => {
    ...
    if (activeTab === 'prompts') {
        if (selectedPack) { 
            ...
            if (!payload.id) payload.order_index = items.length; // Novo no fim
        } else { 
            table = 'products'; 
        }
    }

    if (!payload.id) delete payload.id; // Garante que não envia ID vazio
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 pb-32 animate-fadeIn">
      ...
      {/* === ABA 1: EDITOR VISUAL (PARA TODAS AS PÁGINAS) === */}
      {activeTab === 'editor' && (
        <div className="max-w-6xl space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Coluna 1: Configs Fixas */}
                <div className="lg:col-span-1 space-y-6">
                    {/* Identidade Global (Aparece sempre) */}
                    <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 space-y-4">
                        ...
                        <button onClick={saveIdentity} className="w-full bg-purple-600 text-white font-bold py-2 rounded text-sm hover:bg-purple-500">Salvar Cores/Logo</button>
                    </div>
                ...
                <div className="lg:col-span-2">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => setEditingBlock({ type: 'video' })} className="bg-green-600 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-2 hover:bg-green-500"><Plus size={14}/> Adicionar Bloco</button>
                    </div>
                    ...
                    <div className="space-y-2 max-h-[600px] overflow-y-auto custom-scrollbar pr-2 pb-10">
                        {pageContent.map((block, idx) => (
                            <div 
                                key={block.id} 
                                draggable
                                onDragStart={(e) => handleDragStart(e, idx, 'blocks')}
                                onDragOver={handleDragOver}
                                onDrop={(e) => handleDrop(e, idx, 'blocks')}
                                className="bg-black border border-gray-800 p-3 rounded-lg flex items-center justify-between group hover:border-blue-500 cursor-move"
                            >
                                <div className="flex items-center gap-3 overflow-hidden">
                                    <div className="bg-gray-800 p-1.5 rounded text-gray-500 cursor-grab active:cursor-grabbing"><GripVertical size={16}/></div>
                                ...
                                <div className="flex gap-2">
                                    <button onClick={()=>setEditingBlock(block)} className="text-blue-500 hover:text-white p-1 rounded hover:bg-gray-800"><Edit3 size={16}/></button>
                                    <button onClick={()=>deleteBlock(block.id)} className="text-red-500 hover:text-white p-1 rounded hover:bg-gray-800"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        ))}
                        {pageContent.length === 0 && <div className="text-gray-500 text-center text-sm py-10 border border-dashed border-gray-800 rounded-xl">Nenhum conteúdo. Adicione banners ou vídeos.</div>}
                    </div>
                </div>
            </div>
        </div>
      ...
      {activeTab === 'tutorials' && (
        <div>
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-white">Gerenciar Aulas (Tutoriais)</h2><button onClick={() => setEditingItem({})} className="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg font-bold flex items-center"><Plus size={18} className="mr-2"/> Nova Aula</button></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{items.map((item, index) => (<div key={item.id} draggable onDragStart={(e) => handleDragStart(e, index, 'items')} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, index, 'items')} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden group cursor-move hover:border-blue-500"><div className="aspect-video bg-black relative"><img src={item.thumbnail} className="w-full h-full object-cover opacity-60"/><div className="absolute inset-0 flex items-center justify-center"><Play className="text-white"/></div><div className="absolute top-2 left-2 bg-black/60 p-1 rounded text-white"><GripVertical size={16}/></div></div><div className="p-4"><h3 className="text-white font-bold mb-2 truncate">{item.title}</h3><div className="flex gap-2"><button onClick={()=>setEditingItem(item)} className="flex-1 bg-gray-800 py-2 rounded text-blue-400 text-xs font-bold hover:bg-gray-700">EDITAR</button><button onClick={()=>handleDeleteItem(item.id)} className="flex-1 bg-gray-800 py-2 rounded text-red-400 text-xs font-bold hover:bg-gray-700">EXCLUIR</button></div></div></div>))}</div>
        </div>
      )}
      ...
      {/* MODAL EDIÇÃO BLOCO (PAGE BUILDER) */}
      {editingBlock && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
