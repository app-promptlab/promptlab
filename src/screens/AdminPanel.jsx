...
export default function AdminPanel({ showToast }) {
  ...
  // --- DRAG AND DROP ---
  const handleDragStart = (e, index) => { setDraggedItem(items[index]); e.dataTransfer.effectAllowed = "move"; };
  ...
  const handleSaveItem = async (e) => {
    ...
    if (activeTab === 'prompts') {
        if (selectedPack) { table = 'pack_items'; payload.pack_id = selectedPack.id; if(!payload.order_index) payload.order_index = items.length; } 
        else table = 'products';
    }
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 pb-32 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 space-y-4">
                    <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 space-y-4">
                        <h3 className="text-blue-500 font-bold uppercase text-xs mb-2">Cabeçalho</h3>
            ...
            {!selectedPack ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {items.map((item, index) => (
                        <div key={item.id} draggable onDragStart={(e) => handleDragStart(e, index)} onDragOver={(e) => handleDragOver(e, index)} onDrop={(e) => handleDrop(e, index)} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden group relative cursor-move hover:border-blue-500">
      ...
      {activeTab === 'tutorials' && (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{items.map((item, index) => (<div key={item.id} draggable onDragStart={(e) => handleDragStart(e, index)} onDragOver={(e) => handleDragOver(e, index)} onDrop={(e) => handleDrop(e, index)} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden group cursor-move"><div className="aspect-video bg-black relative"><img src={item.thumbnail} className="w-full h-full object-cover opacity-60"/><div className="absolute inset-0 flex items-center justify-center"><Play className="text-white"/></div><div className="absolute top-2 left-2 bg-black/60 p-1 rounded text-white"><GripVertical size={16}/></div></div><div className="p-4"><h3 className="text-white font-bold mb-2 truncate">{item.title}</h3><div className="flex gap-2"><button onClick={()=>setEditingItem(item)} className="flex-1 bg-gray-800 py-2 rounded text-blue-400 text-xs font-bold">EDITAR</button><button onClick={()=>handleDeleteItem(item.id)} className="flex-1 bg-gray-800 py-2 rounded text-red-400 text-xs font-bold">EXCLUIR</button></div></div></div>))}</div>
        </div>
      )}
