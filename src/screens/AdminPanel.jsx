...
// --- Componente de Upload (Agora com Botão de Excluir) ---
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex items-center gap-3">
         {/* Botão de Upload */}
         <label className={`cursor-pointer bg-gray-900 border border-gray-700 hover:border-gray-500 text-white px-4 py-3 rounded-lg text-sm font-bold flex items-center transition-all flex-1 justify-center ${uploading?'opacity-50':''}`}>
             {uploading ? <Loader2 size={16} className="animate-spin mr-2"/> : <UploadCloud size={16} className="mr-2"/>} {uploading ? '...' : 'Escolher / Trocar'}
             <input type="file" accept="image/*" onChange={uploadImage} className="hidden" disabled={uploading}/>
         </label>

         {/* Preview da Imagem com Botão de Excluir */}
         {currentImage && (
            <div className="relative group h-12 w-12 flex-shrink-0">
                <img src={currentImage} className="h-12 w-12 rounded object-cover border border-gray-600" alt="Preview"/>
                <button 
                    type="button"
                    onClick={() => onUploadComplete('')} // Limpa a imagem
...
export default function AdminPanel({ showToast }) {
  ...
  const saveBlock = async (e) => {
    ...
    if (payload.id) {
        ...
        // INSERT (Sem ID)
        delete payload.id; // Remove ID vazio
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 pb-32 animate-fadeIn">
      ...
      {activeTab === 'editor' && (
        <div className="max-w-6xl space-y-12">
            ...
            {/* CORREÇÃO: SELECT COM FUNDO PRETO PARA TEXTO APARECER */}
            <div className="flex items-center gap-4 mb-6 bg-gray-900 p-4 rounded-xl border border-gray-800">
                ...
                <select 
                    value={selectedPage} 
                    onChange={e => {setSelectedPage(e.target.value); loadPageData(e.target.value);}} 
                    className="bg-black text-white border border-gray-700 p-2 rounded font-bold outline-none cursor-pointer text-lg"
                >
      ...
      {/* MODAIS DE EDIÇÃO (Mantidos com lógica Smart ID) */}
      {editingItem && (
