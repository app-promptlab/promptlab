...
export default function AdminPanel({ updateSettings, settings }) {
  ...
  const handleSave = async (e) => {
      ...
      if (selectedPack && activeSection === 'prompts') {
          ...
          if (!error) { alert('Episódio salvo!'); setEditingItem(null); fetchPackPrompts(selectedPack.id); } 
          return;
      }
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-fadeIn px-6 pt-8">
      ...
      {activeSection === 'settings' && editingItem && (
          <div className="bg-gray-900 p-8 rounded-2xl border border-gray-800 grid grid-cols-1 gap-6">
              ...
              <div><label className="text-gray-400 block mb-2">Posição Logo</label><div className="flex gap-4">{['flex-start','center','flex-end'].map(pos=><button key={pos} onClick={()=>setEditingItem({...editingItem,logo_position:pos})} className={`px-4 py-2 rounded border ${editingItem.logo_position===pos?'bg-blue-600 border-blue-600 text-white':'bg-black border-gray-700 text-gray-400'}`}>{pos}</button>)}</div></div>
      ...
      {activeSection === 'prompts' && (
          selectedPack ? (
                      ...
                      <div className="p-8 grid grid-cols-2 md:grid-cols-5 gap-4">
                          {packPrompts.map(prompt => (
                              <div key={prompt.id} className="bg-black border border-gray-800 rounded-xl overflow-hidden group relative aspect-square">
                                  ...
                                  <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-opacity"><button onClick={()=>setEditingItem(prompt)} className="bg-blue-600 p-2 rounded-full text-white"><Edit3 size={18}/></button><button onClick={()=>handleDelete(prompt.id, true)} className="bg-red-600 p-2 rounded-full text-white"><Trash2 size={18}/></button></div>
                              </div>
                          ))}
                      </div>
                  </div>
              ...
              <div className="animate-fadeIn">
                 <div className="flex justify-between items-center mb-6"><h3 className="text-xl text-white font-bold">Catálogo de Séries</h3><button onClick={() => setEditingItem({ title: '', description: '', price: 'R$ 0,00', cover: '', checkout_url: '' })} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold flex items-center"><Plus size={20} className="mr-2"/> Nova Série</button></div>
          ...
          <div className="animate-fadeIn">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-xl">
                <table className="w-full text-left text-sm text-gray-400"><thead className="bg-black text-xs uppercase font-bold text-gray-500"><tr><th className="p-6">Item</th><th className="p-6">Detalhes</th><th className="p-6 text-right">Ações</th></tr></thead>
                    <tbody className="divide-y divide-gray-800">{dataList.map(item => (<tr key={item.id} className="hover:bg-gray-800/50"><td className="p-6">{activeSection === 'users' ? item.name : item.title}</td><td className="p-6">{activeSection === 'users' ? item.plan : (item.video_url || item.date)}</td><td className="p-6 text-right"><button onClick={() => setEditingItem(item)} className="text-blue-500 mr-2"><Edit3 size={18}/></button>{activeSection !== 'users' && <button onClick={() => handleDelete(item.id)} className="text-red-500"><Trash2 size={18}/></button>}</td></tr>))}</tbody>
                </table>
            </div>
          </div>
      )}
