  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // PROTEÇÃO CONTRA TELA PRETA: Se não achar no banco, usa fallback
        setConfig(pageData || { 
            page_id: pageId, 
      ...
      {config?.show_header && (
        <div className="relative w-full h-[40vh] min-h-[350px] overflow-hidden flex items-center justify-center text-center px-6 bg-black">
            {config.cover_url ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-theme-primary/20 to-black"></div>
            )}
