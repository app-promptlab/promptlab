      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 border-r border-gray-800 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col h-full ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="px-4 pt-4">
            <div className="flex items-center gap-3 p-3 bg-black/40 rounded-xl border border-gray-800">
                <div className="w-10 h-10 rounded-full bg-gray-700 overflow-hidden border border-gray-600"><img src={user?.avatar || ''} className="w-full h-full object-cover" alt="User" /></div>
                <div><h4 className="text-white text-sm font-bold">{user?.name?.split(' ')[0]}</h4><p className="text-theme-primary text-xs font-bold">{user?.plan || 'Free'}</p></div>
            </div>
        </div>
